...
void  eff_pathor_SF(){
 ...
 // TFile *fdataeff= new TFile("reftrigger_data_new.root");
 //  TFile *fMCeff= new TFile("reftrigger_MC_new.root");
...
TFile g("diMuSF.root", "RECREATE"); //create file
   hrEffData1 -> SetName("diMuSF");
   gStyle->SetPaintTextFormat("3.4f ");
   hrEffData1->Draw("TEXT E1 Same");
   hrEffData1 -> Write();
...
//cout << hrEffData1 -> GetXaxis() ->  
  ...
  for (int i(1); i <= nBinsY ; i++) {
        ...
        cout << "i=" << i << " j=" << j << ",  "<< hrEffData1  -> GetBinLowEdge(i) << " -  " << hrEffData1  -> GetBinLowEdge(i+1) << ";  " << hrEffData1  -> GetBinLowEdge(j) << " -  " << hrEffData1  -> GetBinLowEdge(j+1) << ",  " <<  hrEffData1 -> GetBinContent(j,i) << "\n";
...
c-> Print("eff_pathor_SF.pdf"); 

 //  c-> Print("eff_pathor_SF_newMu20.pdf"); 
//c-> Print("eff_pathor_SF_reftrigger.pdf"); 
