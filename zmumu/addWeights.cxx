...
void addWeights(TString treeName="tpTree", TString cut = "") { 
  // Example of cut string:  TString cut="(tag_IsoMu20_eta2p1 || tag_IsoMu20) && tag_combRelIso < 0.15")
    ...
    tData.Draw("tag_nVertices>>hVtxData(100,-0.5,99.5)", "1.*weightLumi");  // add lumi weight to the data
    tMC.Draw("tag_nVertices>>hVtxMC(100,-0.5,99.5)", "1");
   ...
   // TString outFileName = (mcFileName.First(".") > 0 ? mcFileName(0,mcFileName.First(".")) : mcFileName)  + "_WithWeights.root";
      TString outFileName =  "/afs/cern.ch/user/a/agrebeny/eos/cms/store/group/phys_smp/VJets/Bonzai13TeVoutput/tmp/test_WithWeights_new_Lumi.root";
